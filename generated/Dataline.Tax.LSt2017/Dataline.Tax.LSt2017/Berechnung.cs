...
namespace Dataline.Tax.LSt2017
{
    public class Berechnung
    {
        ...
        private void UPTAB17()
        {
            ...
            else
            {
                ...
                {
                    if ((CompareTo(X, 54058m) == (-1)))
                    {
                        Y = Floor(((X - 13769m) / ZAHL10000), 6);
                        RW = (Y * 223.76m);
                        RW = (RW + 2397m);
                        RW = (RW * Y);
                        ST = Floor((RW + 939.57m), 0);
                    }
                    else
                    {
                        if ((CompareTo(X, 256304m) == (-1)))
                        {
                            ST = Floor(((X * 0.42m) - 8475.44m), 0);
                        }
                        else
                        {
                            ST = Floor(((X * 0.45m) - 16164.53m), 0);
                        }
                    }
                }
            }
